...
function page() {
  ...
  const id = localStorage.getItem('Card-Key-Long-Term')
  ...
  const GetPost = () => {
    axios
    .get(`http://localhost:8000/api/v1/rental/view/LongRental/${id}/`, {
      headers: {
  ...
  const HandleFavorite =(id) => {
    axios.post(`http://localhost:8000/api/v1/rental/LongTerm/Favorite/${id}/`, {
  ...
  const RequestCheckAvailability = () => {
    axios.post(`http://localhost:8000/api/v1/rental/request/availability/LongTerm/${id}/`,{
      Name:Name,
      From:startDate,
      to:EndDate,
      Disc:discription
